...
function Detail({ detail, graph, onSelect, zoomedCategory }) {
      ...
      <div className="detail">
        ...
        <p className="lede">
          {zoomedCategory === detail.category.id
            ? `${detail.connected.length} reports in a pop-out above this theme. Hover a report to read its title.`
            : `${detail.connected.length} reports. Activate to fan them out above this theme.`}
        </p>
        <ul className="hits">
          {detail.connected.map((project) => (
            <li key={project.id}>
  ...
  return (
    <div className="detail">
      ...
      <p className="lede">
        {zoomedCategory
          ? "Methods stay on the inner ring. All five category nodes stay on the middle arc — the focused theme stays in place. Its reports fan out in a wider halo above it; hover a report to read its title."
